...
| + признаки-отклонения от агрономических норм культуры (t°/влага/глубина корней) | 0.06351 | 10.95 |

Эксперимент с агрономическими признаками (2026-09-05, `gap_features.py:
CROP_AGRONOMY` + `_add_crop_agronomy_features`): добавлены не сырые
справочные константы (t°/влага/глубина корней культуры), а их отклонение
от уже интерполированной по соседям погоды (`crop_temp_deviation`,
`crop_heat_stress`, `crop_cold_stress`, `crop_water_deficit`,
`crop_water_deficit_root_adjusted`). **Результат: OOF RMSE практически не
изменился** (0.06351 vs 0.06333, разница в пределах шума) — подтверждает
гипотезу, что `crop_type` one-hot + климатология по дню года уже неявно
кодируют бо́льшую часть этого сигнала для восстановления пропусков. Признаки
оставлены в коде (не мешают, документируют эксперимент) как задел для
`anomalies.py` — там явные пороги по культуре могут обоснованно улучшить
именно **интерпретацию причины** аномалии (не саму метрику GapScore),
но это отдельная, ещё не проверенная гипотеза.
