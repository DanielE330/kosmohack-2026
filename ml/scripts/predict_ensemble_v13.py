"""Submission v13: v12, но у каждого участника 12 синтетических масок
вместо 6 (максимум обучающих данных на видимых точках теста при том же
наборе из 5 участников — 3xHGB + 2xLightGBM).
...
MEMBER_PATHS = {
    "private_a12": ROOT / "models/gap_model_private_a12.joblib",
    "private_b12": ROOT / "models/gap_model_private_b12.joblib",
    "private_c12": ROOT / "models/gap_model_private_c12.joblib",
    "private_lgbm_a12": ROOT / "models/gap_model_private_lgbm_a12.joblib",
    "private_lgbm_b12": ROOT / "models/gap_model_private_lgbm_b12.joblib",
}
SUBMISSION_PATH = ROOT / "submissions" / "submission_ensemble_v13.csv"
SUMMARY_PATH = ROOT / "reports/private_adaptation_v13.json"
DIAGNOSTICS_PATH = ROOT / "reports/private_adaptation_v13.csv"
...
def main() -> None:
    ...
    v13 = apply_nonlinear_global_calibration(v4, calibration)

    submission = v13[[ID_COL, DATE_COL]].copy()
    submission[OUTPUT_COL] = v13["v7_prediction"].to_numpy(dtype=float)
    ...
    if len(submission) != expected or submission[OUTPUT_COL].isna().any():
        raise AssertionError("Неверное число строк или NaN в v13 submission")
    if submission.duplicated([ID_COL, DATE_COL]).any():
        raise AssertionError("В v13 submission есть дубликаты")
    if not np.isfinite(submission[OUTPUT_COL]).all():
        raise AssertionError("В v13 submission есть inf")
