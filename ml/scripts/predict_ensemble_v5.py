"""Submission v5: v4 plus a global correction learned on private pseudo-gaps."""
...
SUBMISSION_PATH = ROOT / "submissions" / "submission_ensemble_v5.csv"
DIAGNOSTICS_PATH = ROOT / "reports/private_adaptation_v5.csv"
SUMMARY_PATH = ROOT / "reports/private_adaptation_v5.json"
...
def main() -> None:
    ...
    print("Создаём честные pseudo-gaps на видимых private значениях...", flush=True)
    ...
    v5 = apply_global_calibration(v4, calibration)

    submission = v5[[ID_COL, DATE_COL]].copy()
    submission[OUTPUT_COL] = v5["v5_prediction"].to_numpy(dtype=float)
    ...
    if len(submission) != expected or submission[OUTPUT_COL].isna().any():
        raise AssertionError("Неверное число строк или NaN в v5 submission")
    if submission.duplicated([ID_COL, DATE_COL]).any():
        raise AssertionError("В v5 submission есть дубликаты")
    if not np.isfinite(submission[OUTPUT_COL]).all():
        raise AssertionError("В v5 submission есть inf")
    ...
    summary = {
        ...
        "global_blend_by_crop": GLOBAL_BLEND_BY_CROP,
        "mean_abs_v5_minus_v4": float(
            np.mean(np.abs(v5["v5_prediction"] - v5["v4_prediction"]))
        ),
        "max_abs_v5_minus_v4": float(
            np.max(np.abs(v5["v5_prediction"] - v5["v4_prediction"]))
        ),
    }
