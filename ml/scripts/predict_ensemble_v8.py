"""Experimental v8: conservative extrapolation of the v5 -> v7 direction."""
...
NONLINEAR_SCALE = 1.75
V5_PATH = ROOT / "submissions" / "submission_ensemble_v5.csv"
V7_PATH = ROOT / "submissions" / "submission_ensemble_v7.csv"
OUTPUT_PATH = ROOT / "submissions" / "submission_ensemble_v8.csv"
...
def main() -> None:
    ...
    nonlinear_direction = merged["v7_prediction"] - merged["v5_prediction"]
    prediction = np.clip(
        merged["v5_prediction"] + NONLINEAR_SCALE * nonlinear_direction,
        -1.0,
        1.0,
    )
    ...
    print(
        "Mean abs v8-v7: "
        f"{float(np.mean(np.abs(prediction - merged['v7_prediction']))):.6f}"
    )
