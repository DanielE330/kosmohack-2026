"""Submission v12: v11 плюс два участника LightGBM (алгоритмическая
диверсификация вместо только диверсификации по сидам HGB).
...
SUBMISSION_PATH = ROOT / "submissions" / "submission_ensemble_v12.csv"
SUMMARY_PATH = ROOT / "reports/private_adaptation_v12.json"
DIAGNOSTICS_PATH = ROOT / "reports/private_adaptation_v12.csv"
...
def main() -> None:
    ...
    v12 = apply_nonlinear_global_calibration(v4, calibration)

    submission = v12[[ID_COL, DATE_COL]].copy()
    submission[OUTPUT_COL] = v12["v7_prediction"].to_numpy(dtype=float)
    ...
    if len(submission) != expected or submission[OUTPUT_COL].isna().any():
        raise AssertionError("Неверное число строк или NaN в v12 submission")
    if submission.duplicated([ID_COL, DATE_COL]).any():
        raise AssertionError("В v12 submission есть дубликаты")
    if not np.isfinite(submission[OUTPUT_COL]).all():
        raise AssertionError("В v12 submission есть inf")
