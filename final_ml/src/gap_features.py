...
def make_synthetic_mask(
    ...
    years = dates.dt.year
    target_numeric = pd.to_numeric(work[TARGET_COL], errors="coerce")
    candidates = pd.Series(
        np.isfinite(target_numeric.to_numpy(dtype=float)), index=work.index
    )
...
def _add_neighbor_features(
    ...
    for _, group in work.groupby(ID_COL, sort=False):
        ...
        # ``notna`` alone is not sufficient: +/-inf would still be treated as
        # a valid anchor.  Intersect the logical visibility mask with a strict
        # finite-value mask before harmonic/nearest-neighbour style features
        # consume the arrays.
        numeric_values = pd.to_numeric(group[value_col], errors="coerce")
        effective_known = np.isfinite(numeric_values.to_numpy(dtype=float))
        known = effective_known & ~query
        ...
        known_values = pd.to_numeric(
            known_rows[value_col], errors="coerce"
