...
class CorePipelineTests(unittest.TestCase):
    ...
    def test_natural_nan_and_infinity_are_not_known_anchors(self) -> None:
        frame = pd.DataFrame(
            {
                "anon_polygon_id": ["p"] * 4,
                "date": pd.to_datetime(
                ...
                "primary_ndvi": [0.2, np.nan, np.inf, 0.8],
                "crop_type": ["зерновые"] * 4,
            }
        )
        query = np.array([False, True, False, False])
        features = build_gap_features(frame, query).features.iloc[0]
        self.assertAlmostEqual(features["target_prev1"], 0.2)
        self.assertAlmostEqual(features["target_next1"], 0.8)
        self.assertAlmostEqual(features["target_days_prev1"], 1.0)
        self.assertAlmostEqual(features["target_days_next1"], 2.0)
    ...
    def test_submission_contract(self) -> None:
        ...
        self.assertFalse(submission.primary_ndvi_true.isna().any())
